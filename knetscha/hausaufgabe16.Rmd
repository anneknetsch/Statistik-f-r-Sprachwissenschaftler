...
```
-> Recherche ergab: Lineare Regression erfordert mindestens intervallskslierte Daten. Lineare Regression funktioniert nicht, da es sich bei Farbe um nominalskalierte Daten handelt. Sollte demnach bei Cut und Clarity auch nicht funktionieren, da es sich in diesen Fällen um Ordinalskalen handelt.
...
```
Cut scheint in genauso signifikant für den Preis zu sein wie Color.
...
Man könnte nun jeden einzelnen Faktor noch mit jedem anderen vergleichen. Um das ganze etwas einfacher zu machen, kann man aber auch eine mehrfaktorielle ANOVA berechnen.

```{r}
summary(aov(price~color*cut*clarity,data=diamonds))
```
Haupteffekte (siehe auch oben), sowie Interaktionen sind alle signifikant. 
...
Der Preis eines Diamanten hängt demnach hauptsächlich von der Karatzahl ab. Color, Cut, Clarity und table scheinen eine weniger große Rolle zu spielen.Die Residuals sind bei d Dennoch waren auch für sie die Ergebnisse signifikant.

Man könnte nun noch testen, wie Color, Cut, Clarity und Table mit Carat zusammenhängen.

Mehrfaktorielle ANOVA (carat~color,cut,clarity,table)

```{r}
summary(aov(carat~color*cut*clarity*table,data=diamonds))
```
