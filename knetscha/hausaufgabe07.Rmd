...
for(s in c(3,4,5) ){
  ...
  output2 <- paste("Bei einer Standardabweichung von",s,"bekommen",sehrgut*100,"% eine eins.")
  print(output2)
}
```
...
antwort_hier
Die Anzahl der schlechten Noten ist größer als die Anzahl der guten Noten. Der prozentuale Anzhal beider Fälle steigt mit zunehmender Standardabweichung

Warum?

antwort_hier
Bei den schlechten Noten werden 5 verschiedene Ereignisse (?? Noten: 0,1,2,3,4) betrachtet, bei den guten Noten nur 3 (15,14,13).
...
antwort_hier
Da der Gipfel sich auf der rechten Seite (bei den guten Noten befindet)ist die Verteilung linksschief.
...
(zweiseitiger Test, da Unterschied unklar)
...
m=7
sd=3
n20.z <- sqrt(n)*((m-mu)/sd)
print(n20.z)
```

Das ist ein **insignifikanter** Unterschied. 
(einseitiger Test, da behauptet wird, er bewertet zu streng )
...
Das ist ein **siginifikanter** Unterschied.
(s.o.)
