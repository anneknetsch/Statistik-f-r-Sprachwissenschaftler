...
dat <- read.table("knetscha//body_dim_long.tab",header=TRUE) 
...
# Ja, man erkennt Unterschiede. Die Streuung scheint beim Studiengang Klinische Linguistik 
# am größten zu sein. In den Studiengängen Other und Kognition und Kommunikation scheint 
# die Verteilung sehr regelmäßig zu sein. Im Master Speech Science liegen fast alle in den 
# beiden unteren Quartilen. Es gibt allerdings einen Außreißer nach oben.
...
# sich die Gruppen?
# (Keine explizite Antwort nötig, nur eine Überlegung.)
# Die höchste Dichte im Studiengang Speech Science entspricht in etwa dem Median 
# der im Box Whisker Diagramm angezeigt wurde. Der Ausreißer im Studiengang Speech
# Science fällt hier weniger auf, weil es sich nur um eine Person handelte und 
# der größte Ausschlag anzeigt welche Größe die meisten Personen haben. Dennoch 
# zeigt auch der Ausreißer bei 1,75 m einen relativ großen Ausschlag weil die 
# Stichprobe Speech Science relativ klein ist.  Daher ist es in dieser Darstellung
# auch schwierig die Studiengänge untereinander zu vergleichen. 

# Welche Gruppe hat gefehlt? Wie viele Datenpunkte gab es für die Gruppe?
# (Keine explizite Antwort nötig, nur eine Überlegung.)
# Es fehlen die Gruppen Other und Germanistische Linguistik. Da diese Stichproben
# nur aus einer bwz. zwei Personen bestehen macht eine Berechnung der Dichte keinen
# Sinn, bzw. sie funktioniert nicht.
